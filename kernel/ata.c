...
/* 400ns delay......means read alt status 4 times */
...
/* wait until BSY clears, return 0 if ERR or DF set */
...
{
    /* software reset */
    outb(ATA_PRIMARY_ALT_STATUS, 0x04);
    ata_delay();
    outb(ATA_PRIMARY_ALT_STATUS, 0x00);
    ata_delay();
...
static void ata_setup_lba(u32 lba, u8 count)
{
    ...
    outb(ATA_PRIMARY_LBA_LO,   (u8)(lba & 0xFF));
    outb(ATA_PRIMARY_LBA_MID,  (u8)((lba >> 8)  & 0xFF));
    outb(ATA_PRIMARY_LBA_HI,   (u8)((lba >> 16) & 0xFF));
}
